...
public:
	TestNeoImageCallback() : context_(1), publisher_(context_, ZMQ_PUB) {
        ...
        publisher_.bind("tcp://*:5559");
	streamer.start(8116,4);
	...
	streamer.publish("/mc22", std::string(streamImage.begin(), streamImage.end()));
    }
private:
...
int main() {
	...
	//pink
        //camera.f().ExposureTime.Set(2700.00);
        //camera.f().Gain.Set(1.99);
	
	//black
        //camera.f().ExposureTime.Set(2977.00);
        //camera.f().Gain.Set(1.20);
	...
	//cp_white
	camera.f().ExposureTime.Set(1677.00);
