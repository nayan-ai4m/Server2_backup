...
public:
	TestNeoImageCallback() : context_(1), publisher_(context_, ZMQ_PUB) {
        ...
        publisher_.bind("tcp://*:5568");
	streamer.start(8203,4);
	std::vector<uchar> buff_bgr;
        std::cout << "ZeroMQ publisher bound to tcp://*:5568" << std::endl;
    }
...
int main() {
	...
	//camera.f().Gain.Set(5.95);

	//camera.f().ExposureTime.Set(2500.00);
	//camera.f().Gain.Set(1.97);
	//white
	camera.f().ExposureTime.Set(2500.00);
        camera.f().Gain.Set(1.97);
