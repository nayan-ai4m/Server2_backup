...
public:
	TestNeoImageCallback() : context_(1), publisher_(context_, ZMQ_PUB) {
        ...
        publisher_.bind("tcp://*:5557");
	streamer.start(5114,4);
	...
	streamer.publish("/mc19", std::string(streamImage.begin(), streamImage.end()));
    }
private:
...
int main() {
	...
	//pink
       // camera.f().ExposureTime.Set(4500.00);
        //camera.f().Gain.Set(2.88);

	//red
	//camera.f().ExposureTime.Set(8000.00);
        //camera.f().Gain.Set(2.98);
	
	//black
	//camera.f().ExposureTime.Set(6000.00);
	//camera.f().Gain.Set(1.50);
	
	//white
	camera.f().ExposureTime.Set(6500.00);
	camera.f().Gain.Set(1.99);
	
	camera.f().AcquisitionFrameRateEnable = false;
	camera.f().Width = 1536 ;
	camera.f().Height = 1280 ;
	camera.f().OffsetX = 352 ;
	camera.f().OffsetY = 28;
