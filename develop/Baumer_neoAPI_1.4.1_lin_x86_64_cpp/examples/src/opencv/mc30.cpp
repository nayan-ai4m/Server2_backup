...
public:
	TestNeoImageCallback() : context_(1), publisher_(context_, ZMQ_PUB) {
        ...
        publisher_.bind("tcp://*:5567");
	streamer.start(8206,4);
	std::vector<uchar> buff_bgr;
        std::cout << "ZeroMQ publisher bound to tcp://*:5567" << std::endl;
    }
    ...
    bool success = cv::imencode(".png",img, encodedImage);
	...
	streamer.publish("/mc30", std::string(streamImage.begin(), streamImage.end()));
    }
private:
...
int main() {
	...
	//black
	//camera.f().ExposureTime.Set(5500.00);
	//camera.f().Gain.Set(4.95);

	//cp_white 
	camera.f().ExposureTime.Set(3000.00);
        camera.f().Gain.Set(3.98);
	//camera.f().Width = 1536 ;
	//camera.f().Height = 1536 ;
