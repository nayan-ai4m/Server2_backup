...
public:
	TestNeoImageCallback() : context_(1), publisher_(context_, ZMQ_PUB) {
        ...
        publisher_.bind("tcp://*:5561");
	streamer.start(8200,4);
	...
	streamer.publish("/mc26", std::string(streamImage.begin(), streamImage.end()));
    }
private:
...
int main() {
	...
	//camera.f().Gain.Set(7.00);
	
	//pink
	//camera.f().ExposureTime.Set(6800.00);
	//camera.f().Gain.Set(11.88);

	//cp_white
	camera.f().ExposureTime.Set(4000.00);
