...
public:
	TestNeoImageCallback() : context_(1), publisher_(context_, ZMQ_PUB) {
        // Bind the publisher to a TCP endpoint (e.g., port 5568)
        publisher_.bind("tcp://*:5569");
	streamer.start(8207,4);
	...
	streamer.publish("/mc27", std::string(streamImage.begin(), streamImage.end()));
    }
private:
...
int main() {
	...
	//black
	//camera.f().ExposureTime.Set(2300.00);
