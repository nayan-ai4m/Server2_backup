...
public:
	TestNeoImageCallback() : context_(1), publisher_(context_, ZMQ_PUB) {
        // Bind the publisher to a TCP endpoint (e.g., port 5566)
        publisher_.bind("tcp://*:5566");
	streamer.start(8205,4);
	std::vector<uchar> buff_bgr;
        std::cout << "ZeroMQ publisher bound to tcp://*:5566" << std::endl;
    }
    
	virtual void ImageCallback(const NeoAPI::Image& image) {
        
	    std::cout << "Received image: " << image.GetImageID() << std::endl;
	...
	cv::Mat img(cv::Size(width, height),CV_8UC3, image.GetImageData(), cv::Mat::AUTO_STEP);
        ...
        bool stream = cv::imencode(".jpg",img, streamImage, {cv::IMWRITE_JPEG_QUALITY, 50});
        if (!success) {
        ...
        zmq::message_t message(streamImage.data(), streamImage.size());
        publisher_.send(message, zmq::send_flags::none);
        //cv::imencode(".jpg", img, buff_bgr, params);
        //if (image.GetImageID()%2 == 0)
	streamer.publish("/mc28", std::string(streamImage.begin(), streamImage.end()));
    }
private:
...
int main() {
	...
	//camera.f().ReverseY = true;
	//black_cpp
	//camera.f().ExposureTime.Set(3500.00);
	//camera.f().Gain.Set(1.99);
	
	//pink
	//camera.f().ExposureTime.Set(4500.00);
	//camera.f().Gain.Set(1.5);

	//cp_white
	camera.f().ExposureTime.Set(2000.00);
        camera.f().Gain.Set(1.00);
	...
	camera.f().TriggerSource = NeoAPI::TriggerSource::All;
	camera.f().TriggerActivation.Set(NeoAPI::TriggerActivation::FallingEdge); 
