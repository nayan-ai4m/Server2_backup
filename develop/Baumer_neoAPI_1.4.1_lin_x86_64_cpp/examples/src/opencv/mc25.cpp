...
public:
	TestNeoImageCallback() : context_(1), publisher_(context_, ZMQ_PUB) {
        ...
        publisher_.bind("tcp://*:5564");
	streamer.start(8201,4);
	...
	streamer.publish("/mc25", std::string(streamImage.begin(), streamImage.end()));
    }
private:
...
int main() {
	...
	//black
	//camera.f().ExposureTime.Set(5000.00);
	//camera.f().Gain.Set(3.46);

	//cp_white
	camera.f().ExposureTime.Set(3750.00);
        camera.f().Gain.Set(1.45);
