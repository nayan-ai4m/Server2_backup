...
public:
	TestNeoImageCallback() : context_(1), publisher_(context_, ZMQ_PUB) {
        ...
        publisher_.bind("tcp://*:5560");
	streamer.start(8117,4);
	...
	streamer.publish("/mc21", std::string(streamImage.begin(), streamImage.end()));
    }
private:
...
int main() {
	...
	//pink
        //camera.f().ExposureTime.Set(10000.00);
        //camera.f().Gain.Set(8.04);
