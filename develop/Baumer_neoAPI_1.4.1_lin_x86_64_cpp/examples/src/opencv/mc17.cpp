...
public:
	...
	//cv::Mat img(cv::Size(width, height),CV_8UC1, image.GetImageData(), cv::Mat::AUTO_STEP);
	...
	streamer.publish("/mc17", std::string(streamImage.begin(), streamImage.end()));
    }
private:
...
int main() {
	...
	//tresseme red
	//camera.f().ExposureTime.Set(2500.00);
        //camera.f().Gain.Set(1.98);
