...
public:
	...
	streamer.start(5113,4);
	std::vector<uchar> buff_bgr;
        std::cout << "ZeroMQ publisher bound to tcp://*:5563" << std::endl;
    }
	...
	streamer.publish("/mc18", std::string(streamImage.begin(), streamImage.end()));
    }
private:
...
int main() {
	...
	//camera.f().ExposureTime.Set(6500.00);
        //camera.f().Gain.Set(6.92);
	
        //white
	camera.f().ExposureTime.Set(3304.00);
	camera.f().Gain.Set(2.98);


	//pink 
	//camera.f().ExposureTime.Set(6500.00);
        //camera.f().Gain.Set(2.91);
