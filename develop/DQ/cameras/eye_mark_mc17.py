...
class DetectionProcessor:
    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                cv2.imwrite("may15/"+str(datetime.datetime.now())+".jpeg",image)
    ...
    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
        ...
        event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC18", filename, "defect", "quality")
        tp_data = json.dumps({
    ...
    def check_eyemark_shift(self, boxes, classes, filename, img):
        ...
        if not (lower_bound <= eyemark_centroid_y <= upper_bound):
            self._handle_detection('eyemark_shift', boxes, classes, filename, 'tp25', img)
            return True
        return False
    ...
    def _draw_boxes(self, boxes, classes, img):
        for (x1,y1,x2,y2), cls in zip(boxes, classes):
            name, color = self.models['baumer_hul'].classes.get(int(cls), ("unknown", (255,255,255)))
...
if __name__ == '__main__':
    config = {
        ...
        'models': {
            ...
            'baumer_hul': {'url': '0.0.0.0:8006', 'model_name': 'baumer_hul'}
        },
        'db_str': "postgres://postgres:ai4m2024@localhost:5432/hul?sslmode=disable",
        'zone': (985,585, 1207,685),
        'eyemark_threshold': 5
    }
    ...
    try:
        while True:
            ...
            if socket_mc17 in socks and socks[socket_mc17] == zmq.POLLIN:
                ...
                #cv2.imwrite("may15/"+str(datetime.datetime.now())+".jpeg",frame)
