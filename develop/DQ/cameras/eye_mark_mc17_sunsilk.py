...
class DetectionProcessor:
    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                print(name)
                cv2.imwrite("may21/"+str(datetime.datetime.now())+".jpeg",image)
                if name == 'red_tape' and 0 in classes:
                    ...
                    self.check_solo_eyemark(boxes, classes, filename, img_copy)
    ...
    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
        ...
        event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC17", filename, "Eyemark Shift", "quality")
        tp_data = json.dumps({
        ...
        print(f"{event_data} :: Inserted Successfully ")
        ...
        if self.table_empty:
            ...
            print(f"{tp_data} :: Inserted Successfully ")
            ...
            print(f"{tp_data} :: Updated Successfully ")
    ...
    def _in_zone(self, box):
        ...
        return (zone_x1 <= x1 <= zone_x2 and
                zone_y1 <= y1 <= zone_y2 and
                zone_x1 <= x2 <= zone_x2 and
                zone_y1 <= y2 <= zone_y2)
...
if __name__ == '__main__':
    config = {
        ...
        'zone': (1005,735, 1281,750), #(1108,602, 1162,657),
        'eyemark_threshold': 5
    }
    ...
    try:
        while True:
            ...
            if socket_mc17 in socks and socks[socket_mc17] == zmq.POLLIN:
                ...
                cv2.imwrite("data_may21/"+str(datetime.datetime.now())+".jpeg",frame)
