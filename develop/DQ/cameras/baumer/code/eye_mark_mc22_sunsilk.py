    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                cv2.imwrite("../mc22/may23/inference/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",image)
    ...
    try:
        while True:
            ...
            if socket_mc22 in socks and socks[socket_mc22] == zmq.POLLIN:
                ...
                cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/data_may27/mc22/mc22"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",frame)
