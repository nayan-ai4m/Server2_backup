    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                cv2.imwrite("/home/ai4m/develop/DQ/cameras/baumer/inference/june4/mc20/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%f_%p"))+".jpeg",image)
    ...
    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
       ...
       # event_data = (now, str(uuid.uuid4()), "Baumer Camera", "mc20", filename, prefix, "Quality")
        tp_data = json.dumps({
            ...
            "uuid": str(uuid.uuid4()),
            "active": 1,
            "filepath": 'http://192.168.0.158:8015/'+prefix+'_mc20_'+filename,
            "color_code": 3
        })
    ...
    try:
        while True:
            ...
            if socket_mc20 in socks and socks[socket_mc20] == zmq.POLLIN:
                ...
                #cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/data_may27/june4/mc20/mc20"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%f_%p"))+".png",frame)
