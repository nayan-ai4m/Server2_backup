...
class DetectionProcessor:
    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/mc17/may30/inference/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",image)
    ...
    def check_eyemark_shift(self, boxes, classes, filename, img):
        ...
        if len(eyemarks) > 0:
            ...
            # Only call check_bad_perforation if no perforation is found on x-axis
            if len(perforations) == 0:
                return self.check_bad_perforation(boxes, classes, filename, img)
            if len(perforations) != 0 :
                # Optionally, handle the normal eyemark shift logic here
                eyemark_centroid_y = (eyemark[1] + eyemark[3]) / 2
                nearest_p = min(perforations, key=lambda p: abs(eyemark_centroid_y - (p[1] + p[3]) / 2))
                lower_bound = nearest_p[1] - self.eyemark_threshold
                upper_bound = nearest_p[3] + self.eyemark_threshold
...
if __name__ == '__main__':
    config = {
        ...
        'zone': (110,586,1810,706), #(732,568,1656,660) ,#(986,620, 1298,664)
        'eyemark_threshold': 5
    }
    ...
    try:
        while True:
            ...
            if socket_mc17 in socks and socks[socket_mc17] == zmq.POLLIN:
                ...
                cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/data_may27/mc17_redtape3/mc17"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".png",frame)
