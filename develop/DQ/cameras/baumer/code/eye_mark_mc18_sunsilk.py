    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                cv2.imwrite("../mc18/may23/inference/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",image)
...
if __name__ == '__main__':
    config = {
        ...
        'zone': (615,276, 984,307), # (986,620, 1298,664),
        'eyemark_threshold': 5
    }
    ...
    try:
        while True:
            ...
            if socket_mc18 in socks and socks[socket_mc18] == zmq.POLLIN:
                ...
                cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/data_may27/mc18/mc18"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",frame)
                #cv2.imwrite("../mc18/may23/data_may23/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",frame)
