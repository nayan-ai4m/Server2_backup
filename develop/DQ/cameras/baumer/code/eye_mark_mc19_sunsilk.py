...
class DetectionProcessor:
    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/mc19/may23/inference/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",image)
    ...
    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
        ...
        tp_data = json.dumps({
            ...
            "uuid": str(uuid.uuid4()),
            "active": 1,
            "filepath": 'http://192.168.0.158:8015/'+prefix+'_mc19_'+filename,
            "color_code": 3
        })
...
if __name__ == '__main__':
    config = {
        ...
        'zone': (490,528, 959,638), #(1108,602, 1162,657),
        'eyemark_threshold': 5
    }
    ...
    try:
        while True:
            ...
            if socket_mc19 in socks and socks[socket_mc19] == zmq.POLLIN:
                ...
                cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/data_may27/mc19/mc19"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",frame)
                #cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/mc19/may23/data_may23/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",frame)
