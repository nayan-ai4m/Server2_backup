...
class DBManager:
    ...
    def check_table_empty(self):
        with self.conn.cursor() as cur:
            cur.execute("SELECT COUNT(*) FROM mc29_tp_status")
...
class DetectionProcessor:
    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/test_mc29/may25/inference/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",image)
                
                if name == 'red_tape' and 0 in classes:
                  print('redtap')
                elif name == 'white_sachet':
                    if 0 in classes:
                        self._handle_detection('redtape', boxes, classes, filename, 'tp17', img_copy)
                        print('redtap')
                
                    # Check for perforation_bad detection (class 9)
                    if 9 in classes:
                        self._handle_detection('perforation_missing', boxes, classes, filename, 'tp18', img_copy)
                    
                    # Check for eyemark shift - requires class 3 AND (class 1 OR class 9)
    ...
    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
        output_path = f"{self.output_dir}/{prefix}_mc29_{filename}"
        ...
        event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC29", filename, event_type, "Quality")
       # event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC17", filename, prefix, "Quality")
        tp_data = json.dumps({
            "timestamp": now.isoformat(),
            "uuid": str(uuid.uuid4()),
            "active": 1,
            "filepath": 'http://192.168.0.158:8015/'+prefix+'_mc29_'+filename,
            "color_code": 3
        })
        ...
        if self.table_empty:
            self.db.execute(
                f"INSERT INTO mc29_tp_status({tp_column}) VALUES (%s)",
                (tp_data,)
            )
            self.table_empty = False
        else:
            self.db.execute(
                f"UPDATE mc29_tp_status SET {tp_column} = %s",
                (tp_data,)
            )
...
if __name__ == '__main__':
    config = {
        ...
        'zone': (802,543,1177,576), #(1108,602, 1162,657),
        'eyemark_threshold': 5
    }

    processor = DetectionProcessor(**config)
    context = zmq.Context()
    socket_mc29 = context.socket(zmq.SUB)
    socket_mc29.connect("tcp://localhost:5566")
    socket_mc29.setsockopt_string(zmq.SUBSCRIBE, "")
    poller = zmq.Poller()
    poller.register(socket_mc29, zmq.POLLIN)
    try:
        while True:
            socks = dict(poller.poll())
            if socket_mc29 in socks and socks[socket_mc29] == zmq.POLLIN:
                frame_bytes = socket_mc29.recv()
                frame = cv2.imdecode(np.frombuffer(frame_bytes, dtype=np.uint8), cv2.IMREAD_COLOR)
                processor.process(frame)
                print("receved")
               # cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/test_mc29/may25/data/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",frame)
