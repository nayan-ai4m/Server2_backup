    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/test_mc25/may25/inference/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",image)
    ...
    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
       ...
       # event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC17", filename, prefix, "Quality")
        tp_data = json.dumps({
            ...
            "uuid": str(uuid.uuid4()),
            "active": 1,
            "filepath": 'http://192.168.0.158:8015/'+prefix+'_mc25_'+filename,
            "color_code": 3
        })
    ...
    try:
        while True:
            ...
            if socket_mc25 in socks and socks[socket_mc25] == zmq.POLLIN:
                ...
                print("receved")
               # cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/test_mc25/may25/data/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",frame)
