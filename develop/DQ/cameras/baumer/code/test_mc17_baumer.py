...
class DBManager:
    ...
    def check_table_empty(self):
        with self.conn.cursor() as cur:
            cur.execute("SELECT COUNT(*) FROM mc17_tp_status")
...
class TritonModel:
    ...
    def infer(self, img):
        ...
        return boxes, classes[mask],  scores[mask],img_copy

class DetectionProcessor:
    def __init__(self, input_dir, output_dir, models, db_str, zone, eyemark_threshold,redtape_zone):
        ...
        self.redtape_zone = redtape_zone
        ...
        self.triton_client = grpcclient.InferenceServerClient(url="0.0.0.0:9002")
    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                name, boxes, classes, scores, img_copy = self.q.get()
                
                image = self._draw_boxes(boxes, classes, img_copy.copy())
                cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/mc17/may25/inference/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",image)
                
                if name == 'red_tape' and 0 in classes:
                    #redtape_boxes = [box for box, cls in zip(boxes, classes) if cls == 0 and self._in_specific_zone(box, self.redtape_zone)]
                    
                    # if redtape_boxes:
                    #     self._handle_detection('redtape', boxes, classes, filename, 'tp17', img_copy)
                    #     print('redtap')

                    redtape_boxes = [(box, conf) for box, cls, conf in zip(boxes, classes, scores) 
                        if cls == 0 and self._in_specific_zone(box, self.redtape_zone)
                    ]
                    # Loop through filtered boxes and check confidence
                    for box, conf in redtape_boxes:
                        if conf > 0.6:  # Assuming confidence is a float between 0 and 1
                            self._handle_detection('redtape', [box], [0], filename, 'tp17', img_copy)
                            print("redtape : ",conf)
                            print('redtape')
    ...
    def _run_model(self, name, model, img, filename):
        try:
            boxes, classes, scores, img_copy = model.infer(img)
            self.q.put((name, boxes, classes, scores, img_copy))
        except Exception as e:
            print(f"{name} failed: {e}")

    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
        output_path = f"{self.output_dir}/{prefix}_mc17_{filename}"
        ...
        event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC17", filename, event_type, "quality")
       # event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC17", filename, prefix, "Quality")
        tp_data = json.dumps({
            "timestamp": now.isoformat(),
            "uuid": str(uuid.uuid4()),
            "active": 1,
            "filepath": 'http://192.168.0.158:8015/'+prefix+'_mc17_'+filename,
            "color_code": 3
        })
        ...
        if self.table_empty:
            self.db.execute(
                f"INSERT INTO mc17_tp_status({tp_column}) VALUES (%s)",
                (tp_data,)
            )
            self.table_empty = False
        else:
            self.db.execute(
                f"UPDATE mc17_tp_status SET {tp_column} = %s",
                (tp_data,)
            )
    ...
    def _in_specific_zone(self, box, zone):
        x1, y1, x2, y2 = box
        centroid_x = (x1 + x2) / 2
        centroid_y = (y1 + y2) / 2
        zx1, zy1, zx2, zy2 = zone
        return zx1 <= centroid_x <= zx2 and zy1 <= centroid_y <= zy2
...
if __name__ == '__main__':
    config = {
        ...
        'models': {
            ...
            'red_tape':{'url': '0.0.0.0:8006', 'model_name': 'red_tape'}
        },
        'db_str': "postgres://postgres:ai4m2024@localhost:5432/hul?sslmode=disable",
        'zone': (732,568,1656,660) ,#(986,620, 1298,664), (1108,602, 1162,657),
        'eyemark_threshold': 5,
        'redtape_zone': (6,592, 2034, 1256)
    }
    ...
    try:
        while True:
            ...
            if socket_mc17 in socks and socks[socket_mc17] == zmq.POLLIN:
                ...
                cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/data_may27/mc17_test_pefor/mc17"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".png",frame)
                #cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/mc17/may25/data/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",frame)
