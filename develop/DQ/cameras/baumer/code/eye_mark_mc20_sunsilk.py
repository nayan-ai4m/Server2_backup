...
class DetectionProcessor:
    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                cv2.imwrite("../mc20/may23/inference/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",image)
    ...
    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
        output_path = f"{self.output_dir}/{prefix}_{filename}"
        ...
        #event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC20", filename, "defect", "quality")
        tp_data = json.dumps({
        ...
        self.db.execute(
            "INSERT INTO event_table VALUES (%s,%s,%s,%s,%s,%s,%s)",
            event_data
        )
...
if __name__ == '__main__':
    config = {
        ...
        'zone': (986,620, 1298,664), #(1108,602, 1162,657),
        'eyemark_threshold': 5
    }
    ...
    try:
        while True:
            ...
            if socket_mc20 in socks and socks[socket_mc20] == zmq.POLLIN:
                ...
                cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/data_may27/mc20/mc20"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",frame)
