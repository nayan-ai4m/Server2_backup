    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/mc21/may30/inference/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",image)
    ...
    try:
        while True:
            ...
            if socket_mc21 in socks and socks[socket_mc21] == zmq.POLLIN:
                ...
                cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/data_may27/may31/mc21"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".png",frame)
