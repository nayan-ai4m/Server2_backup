...
class DBManager:
    ...
    def check_table_empty(self):
        with self.conn.cursor() as cur:
            cur.execute("SELECT COUNT(*) FROM mc26_tp_status")
...
class DetectionProcessor:
    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/test_mc26/may23/inference/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",image)
    ...
    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
        ...
        event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC26", filename, "defect", "quality")
        tp_data = json.dumps({
            "timestamp": now.isoformat(),
            "uuid": str(uuid.uuid4()),
            "active": 1,
            "filepath": 'http://192.168.0.158:8015/'+prefix+'_'+filename,
            "color_code": 3
        })
        ...
        if self.table_empty:
            self.db.execute(
                f"INSERT INTO mc26_tp_status({tp_column}) VALUES (%s)",
                (tp_data,)
            )
            self.table_empty = False
        else:
            self.db.execute(
                f"UPDATE mc26_tp_status SET {tp_column} = %s",
                (tp_data,)
            )
...
if __name__ == '__main__':
    ...
    socket_mc26.connect("tcp://localhost:5561")
    ...
    try:
        while True:
            ...
            if socket_mc26 in socks and socks[socket_mc26] == zmq.POLLIN:
                ...
                print("receved")
                cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/mc26/may23/data_may23/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",frame)
