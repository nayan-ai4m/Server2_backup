...
class ZoneTracker:
    def __init__(self, zone_id: str, thresholds: ZoneThresholds):
        ...
        self.history = deque()  # Using deque for efficient appends and pops
        self.last_event_time = None
        self.buffer_time = 20  # Buffer time in seconds
    ...
    def add_measurement(self, count: int, timestamp: float):
        ...
        # Remove old entries beyond buffer time
        while self.history and timestamp - self.history[0]['timestamp'] > self.buffer_time:
            self.history.popleft()  # Remove the oldest entry efficiently
    ...
    def check_conditions(self, current_count: int, zone_status: Dict) -> Tuple[bool, str, int]:
        ...
        all_in_range = all(min_threshold <= h['count'] < max_threshold for h in self.history)
        ...
        elif all_in_range and current_status in [2, 1]:
...
class EnhancedProductionMonitor:
    ...
    def _setup_logging(self):
        ...
        debug_handler = logging.FileHandler('debug_zone_tracking_loop3.log')
    ...
    def _check_zone_conditions(self, zone_id: str, count: int, zone_status: Dict, sensor_id: str) -> None:
        ...
        if not hasattr(self, '_last_summary_time') or current_time - getattr(self, '_last_summary_time', 0) >= 5:
        ...
        elif should_alert and new_status == 0:
            try:
                # Update the status in the zone_status dictionary
                ...
                logging.info(f"Updated status for zone {zone_id} to normal operation (status: {new_status})")
                
            except Exception as e:
                logging.error(f"Error updating status for zone {zone_id}: {e}") 

    # def _generate_zone_summary(self):
    #     """Generate a summary of all zone states"""
    #     summary = ["=== Zone Status Summary ==="]
    #     current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    #     summary.append(f"Time: {current_time}\n")
        
    #     for zone_id, tracker in sorted(self.zone_trackers.items()):
    #         if tracker.history:
    #             latest_count = tracker.history[-1]['count']
    #             measurement_count = len(tracker.history)
    #             zone_type = tracker._get_zone_type()
    #             min_threshold, max_threshold = tracker._get_zone_thresholds()
                
    #             summary.append(f"Zone: {zone_id}")
    #             summary.append(f"  Type: {zone_type}")
    #             summary.append(f"  Current Count: {latest_count}")
    #             summary.append(f"  Measurements in last 30s: {measurement_count}")
    #             summary.append(f"  Thresholds: Min={min_threshold}, Max={max_threshold}")
                
    #             # Add trend analysis
    #             if len(tracker.history) > 1:
    #                 counts = [h['count'] for h in tracker.history]
    #                 trend = "increasing" if counts[-1] > counts[0] else "decreasing" if counts[-1] < counts[0] else "stable"
    #                 summary.append(f"  Trend: {trend}")
                
    #             summary.append("")
        
    #     summary_text = "\n".join(summary)
    #     logging.debug(summary_text)
        
    #     # Save to a separate summary file with timestamp
    #     summary_file = os.path.join(self.log_dir, 
    #                               f"zone_summary_{current_time.replace(':', '-')}.txt")
