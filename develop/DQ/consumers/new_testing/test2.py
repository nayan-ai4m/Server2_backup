...
class ProductionMonitor:
    def __init__(self, config: dict):
        ...
        # Create an array to store events for 30 seconds
        self.zone_event_storage = {zone: [] for zone in self.zone_manager.polygons.keys()}
    ...
    def _check_zone_conditions(self, zone_id: str, count: int, 
        ...
        try:
            ...
            if count < self.zone_manager.thresholds.infeed_min and zone_status.get(zone_id, {}).get("status", 0) in [0, 2]:
                logging.info(f"Zone {zone_id} confirmed possible starvation event")
                ...
                # Store the event in the array for 30 seconds
                self.zone_event_storage[zone_id].append(event_data)
    ...
    def process_message(self, message: Dict[str, Any]):
        ...
        for zone_id, count in zones.items():
            if zone_id in self.zone_manager.polygons and sensor_id in ['17_18', '19_20', '21_22']:
    ...
    def run(self):
        try:
            ...
            while True:
                ...
                # Check if it's been 30 seconds and display stored events for each zone
                if (datetime.now() - self.last_event_time).seconds >= 30:
                    self.display_stored_events()
                    self.last_event_time = datetime.now()
    ...
    def display_stored_events(self):
        for zone_id, events in self.zone_event_storage.items():
            if events:
                logging.info(f"Stored events for zone {zone_id} in the last 30 seconds:")
                for event in events:
                    logging.info(event)
                # Clear the stored events after displaying them
                self.zone_event_storage[zone_id] = []
