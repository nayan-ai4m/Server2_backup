        ...
        if sensorId == "25_26":
            ...
            if zones["mc_26_infeed"] < 2:
                if "mc_26_infeed" in zone_status.keys():
                    if zone_status["mc_26_infeed"]["status"] == 0:
                        ...
                        socket.send_json({"camera":sensorId,"filename":str(datetime.datetime.now())+".jpg"})
                else:
                    zone_status["mc_26_infeed"] = {}
                    zone_status["mc_26_infeed"]["status"] = 1
                ...
                if zones["mc_26_infeed"] > 2: :
                    zone_status["mc_26_infeed"]["status"] = 0
                else:
                    zone_status["mc_26_infeed"] = {}
                    zone_status["mc_26_infeed"]["status"] = 0
            if zones["mc_25_infeed"] < 2:
                if "mc_25_infeed" in zone_status.keys():
                    if zone_status["mc_25_infeed"]["status"] == 0:
                ...
                if "mc_25_infeed" in zone_status.keys():
                        zone_status["mc_25_infeed"]["status"] = 0
                    else:
                        zone_status["mc_25_infeed"] = {}
                        zone_status["mc_25_infeed"]["status"] = 0
