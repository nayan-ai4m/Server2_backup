...
class PLCMonitor:
            ...
            with LogixDriver(config['ip']) as plc:
                ...
                for tag in config['tags']:
                    try:
                        ...
                        # Process tag value - extract SetValue for Front/Rear tags
    ...
    def detect_changes(self, current_readings: dict) -> tuple[bool, list]:
        ...
        for plc_id in PLC_CONFIGS.keys():
            ...
            previous = self.previous_cycle_data.get(plc_id, {})
            
            # Skip comparison if PLC is not connected
            ...
            if previous.get('status') != 'connected':
                ...
                change_log.append(f"{plc_id} - Connected")
                continue
            ...
            for tag, new_value in current_data.items():
                if tag not in previous_data:
                    continue
                    
                old_value = previous_data[tag]
                
                # Compare values with tolerance for numbers
                if isinstance(new_value, (int, float)) and isinstance(old_value, (int, float)):
                    if abs(new_value - old_value) > 0.0001:
                        changes_detected = True
                        if 'Front' in tag or 'Rear' in tag:
                            change_log.append(f"{plc_id} - {tag} SetValue: {old_value} -> {new_value}")
                        else:
                            change_log.append(f"{plc_id} - {tag}: {old_value} -> {new_value}")
