...
class StatusMonitor:
    def __init__(self, db_config):
        ...
        self.tables = ['mc25', 'mc26', 'mc27', 'mc30']
        self.event_tracker_file = 'event_tracker_loop4.json'
        self.unresolved_events = self.load_event_tracker()
        
        # Load fault code messages from Excel
        try:
            excel_path = '/home/ai4m/develop/DQ/plc_tags/Write_events/loop4_status_msg.xlsx'
    ...
    def generate_event_id(self, machine, status, timestamp):
        """Generate event ID in format MC25_20241103_141610_2"""
        time_str = timestamp.strftime("%Y%m%d_%H%M%S")
        return f"{machine.upper()}_{time_str}_{status}"

    def get_fault_message(self, code):
        """Get the fault message for a given fault code"""
        try:
            code = int(code)
    ...
    def insert_event(self, machine, status, fault, timestamp):
        ...
        try:
            ...
            print(f"Issue: {self.get_fault_message(fault)}")
            ...
            try:
                ...
                if current_statuses:
                    for row in current_statuses:
                        ...
                        if first_run:
                            self.handle_initial_state(machine, status, fault, timestamp)
