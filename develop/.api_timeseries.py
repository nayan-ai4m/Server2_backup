...
def insert_device_data(conn, all_telemetry_data):
    ...
    try:
        with conn.cursor() as cursor:
            ...
            for i in range(0, len(all_telemetry_data), batch_size):
                ...
                for data in batch:
                    try:
                        ...
                        success_count += 1
                        successful_updates.append((data['entity_id'], data['raw_ts']))
            ...
            if not mc17_inserted and mc22_data:
                ...
                try:
                    ...
                    conn.commit()
                    success_count += 1
                    successful_updates.append((mc22_data['entity_id'], mc22_data['raw_ts']))
        ...
        return success_count, successful_updates
    except Exception as e:
...
def main():
    ...
    last_processed_ts = {}  # entity_id: max raw_ts processed
        ...
        try:
                            ...
                            for item in telemetry_data['data']:
                                try:
                                    ...
                                    if label and any(num in label for num in target_numbers) and ("Machine No" in label or "M/C" in label):
                                        ...
                                        if entity_id in last_processed_ts and raw_ts <= last_processed_ts[entity_id]:
                                            logging.debug(f"Skipping duplicate or older telemetry for {formatted_label} with raw_ts {raw_ts}")
                                            continue

                                        ist = pytz.timezone("Asia/Kolkata")
                                        if raw_ts <= 0:
                                            logging.warning(f"Invalid telemetry timestamp {raw_ts} for device {entity_id}. Using current time.")
                                            itc_timestamp = datetime.now(ist).replace(microsecond=0)
                                        else:
                                            ...
                                            logging.info(f"Telemetry ts: {raw_ts}, IST: {itc_timestamp}, Current IST: {current_time_ist}, Diff: {(itc_timestamp - current_time_ist).total_seconds()} seconds")
                                            if itc_timestamp > current_time_ist:
                                                logging.warning(f"Telemetry timestamp {itc_timestamp} is in the future. Using current time {current_time_ist} instead.")
                                                itc_timestamp = current_time_ist
                                            itc_timestamp = itc_timestamp.replace(microsecond=0)
            ...
            if telemetry_data_buffer:
                try:
                    ...
                    success_count, successful_updates = insert_device_data(conn, telemetry_data_buffer)
                    operation_watchdog.stop()

                    for entity_id, raw_ts in successful_updates:
                        if entity_id not in last_processed_ts or raw_ts > last_processed_ts[entity_id]:
                            last_processed_ts[entity_id] = raw_ts
