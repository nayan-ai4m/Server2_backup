...
class DetectionProcessor:
    ...
    def _draw_boxes(self, boxes, classes, img):
        for (x1, y1, x2, y2), cls in zip(boxes, classes):
            name, color = self.models['pink_sachet'].classes.get(int(cls), ("custom", (0, 0, 255)))  # Red box
    ...
    # Fix the validation to handle NumPy arrays properly
        ...
        for box, cls in zip(boxes, classes):
            ...
            x1, y1, x2, y2 = box
            name, color = self.models['pink_sachet'].classes.get(int(cls), ("custom", (0, 0, 255)))
            cv2.rectangle(img, (x1, y1), (x2, y2), color, 2)
            cv2.putText(img, name, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
...
if __name__ == '__main__':
    config = {
        ...
        'models': {
            'pink_sachet': {'url': '0.0.0.0:8006', 'model_name': 'pink_sachet'},
