...
class DetectionProcessor:
    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                #cv2.imwrite("/home/ai4m/develop/baumer/inference/mc29/mc29"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%f_%p"))+".jpeg",image)
    ...
    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
       ...
       # event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC17", filename, prefix, "Quality")
        tp_data = json.dumps({
            ...
            "uuid": str(uuid.uuid4()),
            "active": 1,
            "filepath": 'http://192.168.0.185:8015/'+prefix+'_mc29_'+filename,
            "color_code": 3
        })
    ...
    def _draw_boxes(self, boxes, classes, img):
        for (x1,y1,x2,y2), cls in zip(boxes, classes):
            name, color = self.models['pink_sachet'].classes.get(int(cls), ("unknown", (255,255,255)))
    ...
    try:
        while True:
            ...
            if socket_mc29 in socks and socks[socket_mc29] == zmq.POLLIN:
                ...
                print("receved")
               # cv2.imwrite("/home/ai4m/develop/baumer/data/mc29/mc29"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".png",frame)
