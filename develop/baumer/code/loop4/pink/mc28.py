    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                #cv2.imwrite("/home/ai4m/develop/baumer/inference/mc28/mc28"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%f_%p"))+".jpeg",image)
    ...
    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
       ...
       # event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC17", filename, prefix, "Quality")
        tp_data = json.dumps({
            ...
            "uuid": str(uuid.uuid4()),
            "active": 1,
            "filepath": 'http://192.168.0.185:8015/'+prefix+'_mc28_'+filename,
            "color_code": 3
        })
...
if __name__ == '__main__':
    config = {
        ...
        'zone': (614,36,1716,138), #(1108,602, 1162,657),
        'eyemark_threshold': 5
    }
    ...
    try:
        while True:
            ...
            if socket_mc28 in socks and socks[socket_mc28] == zmq.POLLIN:
                ...
                print("receved")
               # cv2.imwrite("/home/ai4m/develop/baumer/data/mc28/mc28"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",frame)
