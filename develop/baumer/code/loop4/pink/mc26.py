...
class DetectionProcessor:
    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                #cv2.imwrite("/home/ai4m/develop/baumer/inference/mc26/mc26"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%f_%p"))+".jpeg",image)
    ...
    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
        ...
        event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC26", filename, event_type, "Quality")
       # event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC17", filename, prefix, "Quality")
        tp_data = json.dumps({
            "timestamp": now.isoformat(),
            "uuid": str(uuid.uuid4()),
            "active": 1,
            "filepath": 'http://192.168.0.185:8015/'+prefix+'_mc26_'+filename,
            "color_code": 3
        })
...
if __name__ == '__main__':
    config = {
        ...
        'zone': (344,478,1362,614), #(1108,602, 1162,657),
        'eyemark_threshold': 5
    }
    ...
    try:
        while True:
            ...
            if socket_mc26 in socks and socks[socket_mc26] == zmq.POLLIN:
                ...
                print("receved")
               # cv2.imwrite("/home/ai4m/develop/baumer/data/mc26/mc26"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",frame)
