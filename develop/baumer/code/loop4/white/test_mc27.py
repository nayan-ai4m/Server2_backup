...
class DBManager:
    ...
    def check_table_empty(self):
        with self.conn.cursor() as cur:
            cur.execute("SELECT COUNT(*) FROM mc27_tp_status")
...
class DetectionProcessor:
    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                image = self._draw_boxes(boxes, classes, img_copy.copy(),conf)
                cv2.imwrite("/home/ai4m/develop/baumer/inference/mc27/mc27"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%f_%p"))+".jpeg",image)
    ...
    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
        output_path = f"{self.output_dir}/{prefix}_mc27_{filename}"
        img_with_boxes = self._draw_boxes(boxes, classes, img.copy(),conf=None)
        cv2.imwrite(output_path, img_with_boxes)

        now = datetime.datetime.now()
        event_type = self.event_mapping.get(prefix, prefix)  # fallback to raw prefix if not mapped
        event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC27", filename, event_type, "quality")
       # event_data = (now, str(uuid.uuid4()), "Baumer Camera", "mc27", filename, prefix, "Quality")
        tp_data = json.dumps({
            "timestamp": now.isoformat(),
            "uuid": str(uuid.uuid4()),
            "active": 1,
            "filepath": 'http://192.168.0.185:8015/'+prefix+'_mc27_'+filename,
            "color_code": 3
        })
        ...
        if self.table_empty:
            self.db.execute(
                f"INSERT INTO mc27_tp_status({tp_column}) VALUES (%s)",
                (tp_data,)
            )
            self.table_empty = False
        else:
            self.db.execute(
                f"UPDATE mc27_tp_status SET {tp_column} = %s",
                (tp_data,)
            )
    ...
    def _draw_boxes(self, boxes, classes, img, conf=None):
        ...
        # Draw zone rectangle
        zone_x1, zone_y1, zone_x2, zone_y2 = self.zone
        cv2.rectangle(img, (zone_x1, zone_y1), (zone_x2, zone_y2), (0, 255, 255), 2)
        cv2.putText(img, "Zone", (zone_x1, zone_y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
...
if __name__ == '__main__':
    config = {
        ...
        'zone': (698,36,1660,154), #(732,568,1656,660) ,#(986,620, 1298,664)
        'eyemark_threshold': 5
    }

    processor = DetectionProcessor(**config)
    context = zmq.Context()
    socket_mc27 = context.socket(zmq.SUB)
    socket_mc27.connect("tcp://localhost:5569")
    socket_mc27.setsockopt_string(zmq.SUBSCRIBE, "")
    poller = zmq.Poller()
    poller.register(socket_mc27, zmq.POLLIN)
    try:
        while True:
            socks = dict(poller.poll())
            if socket_mc27 in socks and socks[socket_mc27] == zmq.POLLIN:
                frame_bytes = socket_mc27.recv()
                frame = cv2.imdecode(np.frombuffer(frame_bytes, dtype=np.uint8), cv2.IMREAD_COLOR)
                frame = cv2.flip(frame, -1)
                print(frame.shape)
                processor.process(frame)
                #cv2.imwrite("/home/ai4m/develop/baumer/data/mc27/mc27"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%f_%p"))+".png",frame)
                #cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/mc27/may25/data/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",frame)
