"""
Loop4 Slitting Monitoring (MC25–MC30)

Real-time monitoring of slitting across MC25–MC30:
- Captures frames from ZMQ camera feed
- Runs Triton inference to detect slitting defects in target zone
- Computes batch-wise average slitting defects
- Saves low-slitting images locally and generates public URL
...
# ========= LOAD CONFIG ==========
with open("loop4_config.json", "r") as f:
    config = json.load(f)
...
# ========= COUNT SLITTING ==========
def count_slitting(boxes, classes, zone_polygon, target_class):
    count = 0
...
def insert_status(machine_cfg, avg_count, tp_db, event_db, base_url, frame=None):
    ...
    if active == 1 and frame is not None:
        ...
        filename = f"low_slitting_{camera_id}_{timestamp_str}.jpeg"
...
def run_machine(machine_cfg, model_cfg, db_cfg):
    ...
    for frame in frame_gen:
        ...
        slitting_count = count_slitting(boxes, classes, zone, machine_cfg["target_class"])
        slitting_counts.append(slitting_count)

        print(f"[{camera_id}] Frame {frame_idx} | Count: {slitting_count}")

        # Compute average every batch
        if len(slitting_counts) == machine_cfg["batch_size"]:
            avg_count = np.mean(slitting_counts)
            print(f"[{camera_id}] 📊 Avg slitting ({machine_cfg['batch_size']} frames): {avg_count:.2f}")
            insert_status(machine_cfg, avg_count, db_cfg["tp_db"], db_cfg["event_db"], BASE_URL, frame)
            slitting_counts.clear()
