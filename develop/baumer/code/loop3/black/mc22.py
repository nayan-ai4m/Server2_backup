...
class DBManager:
    ...
    def check_table_empty(self):
        with self.conn.cursor() as cur:
            cur.execute("SELECT COUNT(*) FROM mc22_tp_status")
...
class DetectionProcessor:
    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                #cv2.imwrite("/home/ai4m/develop/baumer/inference/mc22/mc22"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%f_%p"))+".jpeg",image)
    ...
    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
        output_path = f"{self.output_dir}/{prefix}_mc22_{filename}"
        ...
        event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC22", filename, event_type, "Quality")
       # event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC22", filename, prefix, "Quality")
        tp_data = json.dumps({
            "timestamp": now.isoformat(),
            "uuid": str(uuid.uuid4()),
            "active": 1,
            "filepath": 'http://192.168.0.185:8015/'+prefix+'_mc22_'+filename,
            "color_code": 3
        })
        ...
        if self.table_empty:
            self.db.execute(
                f"INSERT INTO mc22_tp_status({tp_column}) VALUES (%s)",
                (tp_data,)
            )
            self.table_empty = False
        else:
            self.db.execute(
                f"UPDATE mc22_tp_status SET {tp_column} = %s",
                (tp_data,)
            )
    ...
    def check_eyemark_shift(self, boxes, classes, filename, img,conf):
        ...
        perforations = [box for box, cls,cf in zip(boxes, classes,conf) if cls == 0 and self._in_zone(box)]
    ...
    #         name, color = self.models['sunsilk_black'].classes.get(int(cls), ("custom", (0, 0, 255)))  # Red box
    #         cv2.rectangle(img, (x1, y1), (x2, y2), color, 2)
    #         cv2.putText(img, name, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
    #     return img
    """def draw_zones(self, img):
    # Draw main zone (self.zone)
        zx1, zy1, zx2, zy2 = self.zone
        cv2.rectangle(img, (zx1, zy1), (zx2, zy2), (255, 0, 0), 2)
        cv2.putText(img, "Main Zone", (zx1, zy1 - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)

        # Draw horizontal zone
        hz1, hy1, hz2, hy2 = (385,519,1768,674)
        cv2.rectangle(img, (hz1, hy1), (hz2, hy2), (0, 165, 255), 2)
        cv2.putText(img, "Horizontal Zone", (hz1, hy1 - 10),
...
if __name__ == '__main__':
    config = {
        ...
        'zone': (385,519,1768,674), #(732,568,1656,660) ,#(986,620, 1298,664)
        'eyemark_threshold': 10
    }

    processor = DetectionProcessor(**config)
    context = zmq.Context()
    socket_mc22 = context.socket(zmq.SUB)
    socket_mc22.connect("tcp://localhost:5559")
    socket_mc22.setsockopt_string(zmq.SUBSCRIBE, "")
    poller = zmq.Poller()
    poller.register(socket_mc22, zmq.POLLIN)
    try:
        while True:
            socks = dict(poller.poll())
            if socket_mc22 in socks and socks[socket_mc22] == zmq.POLLIN:
                frame_bytes = socket_mc22.recv()
                frame = cv2.imdecode(np.frombuffer(frame_bytes, dtype=np.uint8), cv2.IMREAD_COLOR)
                frame = cv2.flip(frame, -1)
                print(frame.shape)
                processor.process(frame)
               # cv2.imwrite("/home/ai4m/develop/baumer/data/mc22/mc22"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%f_%p"))+".png",frame)
                #cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/mc22/may25/data/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",frame)
