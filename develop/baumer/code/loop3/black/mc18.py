...
class DetectionProcessor:
    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                #cv2.imwrite("/home/ai4m/develop/baumer/inference/mc18/mc18"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%f_%p"))+".jpeg",image)
    ...
    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
       ...
       # event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC18", filename, prefix, "Quality")
        tp_data = json.dumps({
            ...
            "uuid": str(uuid.uuid4()),
            "active": 1,
            "filepath": 'http://192.168.0.185:8015/'+prefix+'_mc18_'+filename,
            "color_code": 3
        })
    ...
    def check_eyemark_shift(self, boxes, classes, filename, img,conf):
        ...
        horizontal_zone = (321,491,1308,714)  # (x1, y1, x2, y2)
    ...
    # Draw main zone (self.zone)
        ...
        hz1, hy1, hz2, hy2 = (171, 480, 1895, 696)
...
if __name__ == '__main__':
    config = {
        ...
        'zone': (366,188,1210,354), #(359,248,1265,309),
    ...
    try:
        while True:
            ...
            if socket_mc18 in socks and socks[socket_mc18] == zmq.POLLIN:
               ...
               # cv2.imwrite("/home/ai4m/develop/baumer/data/mc18/mc18"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%f_%p"))+".png",frame)
