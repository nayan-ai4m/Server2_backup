...
class DBManager:
    ...
    def check_table_empty(self):
        with self.conn.cursor() as cur:
            cur.execute("SELECT COUNT(*) FROM mc19_tp_status")
...
class DetectionProcessor:
    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
               ...
               # cv2.imwrite("/home/ai4m/develop/baumer/inference/mc19/mc19"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%f_%p"))+".jpeg",image)
    ...
    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
        output_path = f"{self.output_dir}/{prefix}_mc19_{filename}"
        ...
        event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC19", filename, event_type, "Quality")
       # event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC19", filename, prefix, "Quality")
        tp_data = json.dumps({
            "timestamp": now.isoformat(),
            "uuid": str(uuid.uuid4()),
            "active": 1,
            "filepath": 'http://192.168.0.185:8015/'+prefix+'_mc19_'+filename,
            "color_code": 3
        })
        ...
        if self.table_empty:
            self.db.execute(
                f"INSERT INTO mc19_tp_status({tp_column}) VALUES (%s)",
                (tp_data,)
            )
            self.table_empty = False
        else:
            self.db.execute(
                f"UPDATE mc19_tp_status SET {tp_column} = %s",
                (tp_data,)
            )
...
if __name__ == '__main__':
    config = {
        ...
        'zone': (32,334,1426,522), #(732,568,1656,660) ,#(986,620, 1298,664)
        'eyemark_threshold': 5
    }

    processor = DetectionProcessor(**config)
    context = zmq.Context()
    socket_mc19 = context.socket(zmq.SUB)
    socket_mc19.connect("tcp://localhost:5557")
    socket_mc19.setsockopt_string(zmq.SUBSCRIBE, "")
    poller = zmq.Poller()
    poller.register(socket_mc19, zmq.POLLIN)
    try:
        while True:
            socks = dict(poller.poll())
            if socket_mc19 in socks and socks[socket_mc19] == zmq.POLLIN:
                frame_bytes = socket_mc19.recv()
                frame = cv2.imdecode(np.frombuffer(frame_bytes, dtype=np.uint8), cv2.IMREAD_COLOR)
                print(frame.shape)
                processor.process(frame)
                #cv2.imwrite("/home/ai4m/develop/baumer/data/mc19/mc19"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%f_%p"))+".png",frame)
                #cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/mc19/may25/data/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",frame)
