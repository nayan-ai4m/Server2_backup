    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
               ...
               # cv2.imwrite("/home/ai4m/develop/baumer/inference/mc21/mc21"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%f_%p"))+".jpeg",image)
    ...
    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
       ...
       # event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC21", filename, prefix, "Quality")
        tp_data = json.dumps({
            ...
            "uuid": str(uuid.uuid4()),
            "active": 1,
            "filepath": 'http://192.168.0.185:8015/'+prefix+'_mc21_'+filename,
            "color_code": 3
        })
...
if __name__ == '__main__':
    config = {
        ...
        'zone': (856,367,1302,417), #(732,568,1656,660) ,#(986,620, 1298,664)
        'eyemark_threshold': 5
    }
    ...
    try:
        while True:
            ...
            if socket_mc21 in socks and socks[socket_mc21] == zmq.POLLIN:
                ...
                #cv2.imwrite("/home/ai4m/develop/baumer/data/mc21/mc21"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%f_%p"))+".png",frame)
                #cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/mc21/may25/data/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",frame)
