...
class DetectionProcessor:
    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                cv2.imwrite("/home/ai4m/develop/baumer/inference/mc17/mc17"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%f_%p"))+".jpeg",image)
    ...
    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
       ...
       # event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC17", filename, prefix, "Quality")
        tp_data = json.dumps({
            ...
            "uuid": str(uuid.uuid4()),
            "active": 1,
            "filepath": 'http://192.168.0.185:8015/'+prefix+'_mc17_'+filename,
            "color_code": 3
        })
    ...
    def _in_zone(self, box):
        ...
        # Polygon must be a numpy array of shape (N, 1, 2)
        polygon = np.array(self.zone, dtype=np.int32).reshape((-1, 1, 2))
...
if __name__ == '__main__':
    config = {
        ...
        'db_str': "postgres://postgres:ai4m2024@192.168.1.149:5432/hul?sslmode=disable",
        'zone': [(149,303),(829,244),(1365,286),(1363,320),(821,285),(155,344)],#(104,526,2018,580), #(732,568,1656,660) ,#(986,620, 1298,664)
        'eyemark_threshold': 5
    }
    ...
    try:
        while True:
            ...
            if socket_mc17 in socks and socks[socket_mc17] == zmq.POLLIN:
                ...
                #cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/data_may27/may31/mc17"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%f_%p"))+".png",frame)
