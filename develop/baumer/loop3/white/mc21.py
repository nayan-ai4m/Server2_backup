    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                # Save detection image to self.output_dir instead of hardcoded path
                cv2.imwrite(f"{self.output_dir}/{name}_mc21_{filename}", image)
...
if __name__ == '__main__':
    ...
    socket_mc21.connect(config['zmq']['address'])
