    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                # Save detection image to self.output_dir
                cv2.imwrite(f"{self.output_dir}/{name}_{self.machine_id}_{filename}", image)
...
if __name__ == '__main__':
    ...
    # Validate that all required model parameters are present
    ...
    socket_mc22.connect(config['zmq']['address'])
    ...
    try:
        while True:
            ...
            if socket_mc22 in socks and socks[socket_mc22] == zmq.POLLIN:
                ...
                cv2.imwrite(f"{config['output_dir']}/{MACHINE_ID}_{str(datetime.datetime.now().strftime('%m_%d_%Y_%H_%M_%S_%f_%p'))}.png", frame)
