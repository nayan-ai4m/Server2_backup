...
def insert_status(machine_cfg, avg_count, tp_db, event_db, base_url, frame=None):
    ...
    if active == 1 and frame is not None:
        ...
        file_url = f"{base_url}/{filename}" if base_url else output_path

        print(f"[{camera_id}] ⚠️ Low perforation → Saved {output_path}")
...
def run_machine(machine_cfg, model_cfg, db_cfg):
    ...
    print(f"[{camera_id}] 🚀 Started worker (ZMQ MODE) on port {machine_cfg['zmq_port']}")
    ...
    for frame in frame_gen:
        ...
        # cv2.polylines(draw_frame, [zone], isClosed=True, color=(0, 255, 255), thickness=2)
        # for box, cls in zip(boxes, classes):
        #     x1, y1, x2, y2 = box
        #     if cls == machine_cfg['target_class']:
        #         cx, cy = int((x1 + x2) / 2), int((y1 + y2) / 2)
        #         inside = cv2.pointPolygonTest(zone, (cx, cy), False)
        #         color = (0, 255, 0) if inside >= 0 else (0, 0, 255)
        #         cv2.rectangle(draw_frame, (x1, y1), (x2, y2), color, 2)
        #         cv2.circle(draw_frame, (cx, cy), 4, color, -1)

        # frame_name = f"{camera_id}_frame_{frame_idx:06d}.jpg"
        # out_path = os.path.join(output_dir, frame_name)
        # cv2.imwrite(out_path, draw_frame)

        # ====== Average calculation every batch ======
