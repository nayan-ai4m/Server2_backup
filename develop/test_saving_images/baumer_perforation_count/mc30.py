...
config = {
    ...
    'zone': np.array([
        (20,450),(2004,485),(2010,690),(15,654)
...
def insert_status(avg_count, tp_db_config, event_db_config, filepath=""):
    ...
    if active == 1 and frame is not None:
        ...
        filename = f"low_perforation_mc30_{timestamp_str}.jpeg"
    ...
    try:
        ...
        cur_tp.execute("UPDATE mc30_tp_status SET tp18 = %s", (json.dumps(tp_data),))
        if cur_tp.rowcount == 0:
            cur_tp.execute("INSERT INTO mc30_tp_status (tp18) VALUES (%s)", (json.dumps(tp_data),))
        conn_tp.commit()
        cur_tp.close()
        conn_tp.close()
