...
def insert_status(avg_count, tp_db_config, event_db_config, filepath=""):
    ...
    if active == 1 and frame is not None:
        ...
        filename = f"low_perforation_mc22_{timestamp_str}.jpeg"
    ...
    try:
        ...
        cur_tp.execute("UPDATE mc22_tp_status SET tp18 = %s", (json.dumps(tp_data),))
        if cur_tp.rowcount == 0:
            cur_tp.execute("INSERT INTO mc22_tp_status (tp18) VALUES (%s)", (json.dumps(tp_data),))
        conn_tp.commit()
        cur_tp.close()
        conn_tp.close()
