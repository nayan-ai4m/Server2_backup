...
config = {
    ...
    'zone': np.array([
	(20, 329), (2011, 329), (2022, 521), (27, 593)
        #(21,195),(2034,195),(2034,576),(21,576)
...
def insert_status(avg_count, tp_db_config, event_db_config, filepath=""):
    ...
    if active == 1 and frame is not None:
        ...
        filename = f"low_perforation_mc26_{timestamp_str}.jpeg"
    ...
    try:
        ...
        cur_tp.execute("UPDATE mc26_tp_status SET tp18 = %s", (json.dumps(tp_data),))
        if cur_tp.rowcount == 0:
            cur_tp.execute("INSERT INTO mc26_tp_status (tp18) VALUES (%s)", (json.dumps(tp_data),))
        conn_tp.commit()
        cur_tp.close()
        conn_tp.close()
