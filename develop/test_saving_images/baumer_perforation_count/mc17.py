...
config = {
    ...
    'zone': np.array([
        (24,345),(2028,345),(2028,594),(24,594)
...
def insert_status(avg_count, tp_db_config, event_db_config, filepath=""):
    ...
    try:
        ...
        cur_tp.execute("UPDATE mc17_tp_status SET tp18 = %s", (json.dumps(tp_data),))
        if cur_tp.rowcount == 0:
            cur_tp.execute("INSERT INTO mc17_tp_status (tp18) VALUES (%s)", (json.dumps(tp_data),))
        conn_tp.commit()
        cur_tp.close()
        conn_tp.close()
