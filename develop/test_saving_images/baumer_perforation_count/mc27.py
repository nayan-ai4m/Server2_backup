...
config = {
    ...
    'zone': np.array([
        (108, 27), (2017, 25), (2019, 233), (117, 221)
...
def insert_status(avg_count, tp_db_config, event_db_config, filepath=""):
    ...
    if active == 1 and frame is not None:
        ...
        filename = f"low_perforation_mc27_{timestamp_str}.jpeg"
    ...
    try:
        ...
        cur_tp.execute("UPDATE mc27_tp_status SET tp18 = %s", (json.dumps(tp_data),))
        if cur_tp.rowcount == 0:
            cur_tp.execute("INSERT INTO mc27_tp_status (tp18) VALUES (%s)", (json.dumps(tp_data),))
        conn_tp.commit()
        cur_tp.close()
        conn_tp.close()
