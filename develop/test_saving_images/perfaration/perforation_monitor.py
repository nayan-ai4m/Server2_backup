"""
Perforation Monitoring System (Unified Configurable)

Real-time monitoring of Perforation across multiple machines:
...
Usage:
    python perforation_monitoring.py [config_file]

    Example:
        python perforation_monitoring.py perforation_config.json
        python perforation_monitoring.py  (uses perforation_config.json by default)
"""
...
# ========= LOAD CONFIG ==========
if len(sys.argv) < 2:
    config_file = "perforation_config.json"
...
models_cfg = config["models"]  # Dictionary of models
db_cfg = config["db"]
...
print(f"   Models available: {', '.join(models_cfg.keys())}")
...
# ========= MACHINE WORKER ==========
def run_machine(machine_cfg, models_cfg, db_cfg):
    camera_id = machine_cfg["camera_id"]
    output_dir = machine_cfg["output_dir"]
    os.makedirs(output_dir, exist_ok=True)

    # Get the specific model config for this machine
    model_name = machine_cfg.get("model", "pink_sachet")  # Default to pink_sachet if not specified
    model_cfg = models_cfg[model_name]

    print(f"[{camera_id}] 🚀 Started worker on port {machine_cfg['zmq_port']} using model '{model_name}'")
...
if __name__ == "__main__":
    print(f"🚀 Starting Perforation Monitoring System...")
    print(f"   Workers: {len(machines)}")
    print()

    with ThreadPoolExecutor(max_workers=len(machines)) as executor:
        futures = [executor.submit(run_machine, m, models_cfg, db_cfg) for m in machines]
        for f in futures:
            f.result()
