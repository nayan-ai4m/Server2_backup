...
class CameraConfig:
    ...
    CAMERA_STREAMS: Dict[str, str] = {
        ...
        "l4_taping_view1": "rtsp://admin:unilever2024@192.168.1.26:554/Streaming/Channels/101",
        "l3_EOL": "rtsp://admin:unilever2024@192.168.1.29:554/Streaming/Channels/101",
        "l4_EOL": "rtsp://admin:unilever2024@192.168.1.28:554/Streaming/Channels/101",
        "mc17": "rtsp://admin:unilever2024@192.168.1.21:554/Streaming/Channels/101",
        "mc19": "rtsp://admin:unilever2024@192.168.1.20:554/Streaming/Channels/101",
        "mc21": "rtsp://admin:unilever2024@192.168.1.22:554/Streaming/Channels/101",
        "mc25": "rtsp://admin:unilever2024@192.168.1.27:554/Streaming/Channels/101",
        "mc27": "rtsp://admin:unilever2024@192.168.1.25:554/Streaming/Channels/101",
        "mc29": "rtsp://admin:unilever2024@192.168.1.23:554/Streaming/Channels/101",
        
        # H264 streams - Quality Control Cameras
        "l3_taping_view2": "rtsp://admin:unilever2024@192.168.1.31:554/Streaming/Channels/101",
        "l4_taping_view2": "rtsp://admin:unilever2024@192.168.1.32:554/Streaming/Channels/101",
        "l3_highbay": "rtsp://admin:unilever2024@192.168.1.30:554/Streaming/Channels/101",
        "l4_highbay": "rtsp://admin:unilever2024@192.168.1.33:554/Streaming/Channels/101",
        "l3_taping_view1": "rtsp://admin:unilever2024@192.168.1.24:554/Streaming/Channels/101",
    }
...
class CameraMappingService:
    ...
    def __init__(self, config: CameraConfig = None):
        ...
        self.logger.info("Camera Mapping Service initialized successfully")
    ...
    def capture_frame_with_retry(self, machine_name: str, status: str) -> bool:
        """
    ...
    def _capture_single_frame(self, rtsp_url: str, camera_name: str, 
                             machine_name: str, status: str, attempt: int) -> bool:
        """
        ...
        try:
            ...
            filename = f"{status}_{machine_name}_{timestamp}.{self.config.IMAGE_FORMAT}"
    ...
    def start_kafka_consumer(self) -> None:
        """
        Start Kafka consumer with proper error handling and reconnection logic.
        
        Implements AI4M standard for resilient message consumption.
        """
        consumer = None
    ...
    def _process_kafka_message(self, message) -> None:
        ...
        try:
            ...
            self.logger.info(f"Processing event: machine={machine_name}, status={status}, ts={timestamp}")
            
            if not machine_name:
                self.logger.warning("Received event without machine_name")
                return

            # ✅ Check if status should trigger capture
            if not self._should_capture_image(status):
                self.logger.info(f"Skipping capture (non-fault event): {status}")
                return
            
            # Trigger frame capture
            capture_success = self.capture_frame_with_retry(machine_name, status)
            
            if capture_success:
                self.logger.info(f"Event processed successfully: {machine_name}")
            else:
                self.logger.warning(f"Frame capture failed for event: {machine_name}")
...
def main():
    ...
    try:
        ...
        logger.info("Starting AI4M Camera Mapping System")
