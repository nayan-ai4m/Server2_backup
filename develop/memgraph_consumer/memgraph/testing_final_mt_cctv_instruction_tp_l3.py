...
def get_table(machine):
    ...
    if machine in ['pre_taping_l3', 'l3_outfeed']:
        return 'press_tp_status'
    if machine in ['l3_tapping_outfeed', 'l3_tapping_postoutfeed']:
        return 'tpmc_tp_status'
    if machine == 'check_weigher_rejection_l3':
        return 'check_weigher_tp_status'
    if machine in ['highbay_l3', 'cw_outfeed_l3']:
        return 'highbay_tp_status'
    if machine == 'case_erector_conveyor_l3':
...
def get_tp_column(status, orient, part):
    ...
    if part in ['l3_tapping_outfeed', 'l3_tapping_postoutfeed'] and 'bad_orientation' in [status, orient]:
        return 'tp24'
    if part in ['pre_taping_l3', 'outfeed_l3c4_l3'] and 'bad_orientation' in [status, orient]:
        return 'tp31'
    if part == 'l3_outfeed' and 'jamming' in status and 'possible_jamming' not in status:
        return 'tp32'
    if part == 'check_weigher_rejection_l3' and 'jamming' in status and 'possible_jamming' not in status:
        return 'tp28'
    if part in ['case_erector_conveyor_l3'] and 'jamming' in status and 'possible_jamming' not in status:
        return 'tp40'
    if part in ['highbay_l3', 'cw_outfeed_l3'] and 'jamming' in status and 'possible_jamming' not in status:
        return 'tp30'
...
def normalize_part(machine: str) -> str:
    m = machine.lower()
    if m in ['l3_tapping_outfeed', 'pre_taping_l3',
             'l3_tapping_postoutfeed', "l3_outfeed",
             "check_weigher_rejection_l3", "case_erector_conveyor_l3",
             "highbay_l3", "cw_outfeed_l3"]:
        return m
    ...
    while True:
        try:
            ...
            for msg in consumer:
                try:
                    update_status(
                        msg.value.get('machine_name'),
                        msg.value.get('status'),
                        msg.value.get('orientation')
                    )
