...
def extract_badloop3_from_objects(msg):
    try:
        if isinstance(msg, dict) and "objects" in msg:
            for obj in msg["objects"]:
                fields = obj.split("|")
                if len(fields) >= 13:
                    location = fields[8]  # sensor/machine name
                    tags = fields[12:]   # values after '#'
                    if location == "pre_taping_l3":
                        for tag in tags:
                            if "badloop3" in tag:
                                return tag  # return first match like badloop3_mat
        return None
    except Exception as e:
        print(f"[ERROR] extract_badloop3_from_objects failed: {e}")
        return None
        ...
        for machine_name in machine_names:
            if machine_name in zone_nodes:
                ...
                #        new_status = "nothing"
                    ...
                    # extract badloop3_mat from objects
                    badloop_extracted = extract_badloop3_from_objects(msg)
                    if badloop_extracted:
                        orientation = badloop_extracted
                        print(f"[DEBUG] Extracted orientation from object tags: {orientation}")

                    #print(f"[DEBUG] Processing: {machine_name}")
                    #print(f"[DEBUG] Orientation received: {orientation}")

                    if "badloop3" in orientation.lower():
                        print(f"[DEBUG] Detected 'badloop3' → machine_name: {machine_name}, orientation: {orientation}, cld_status: {cld_status}, cld_count: {cld_count}, source_id: {source_id}")
                        new_status = "bad_orientation"

                    elif "goodloop3" in orientation.lower():
                        print(f"[DEBUG] Detected 'goodloop3' → machine_name: {machine_name}, orientation: {orientation}")
                        new_status = "tttt"

                    else:
                        #print(f"[DEBUG] No loop match → machine_name: {machine_name}, orientation: {orientation}")
                        new_status = "nothing"
