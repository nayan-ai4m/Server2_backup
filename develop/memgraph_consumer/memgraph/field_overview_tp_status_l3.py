...
FIELD_TPS = [
    ...
    "tp21", "tp22", "tp72", "tp24", "tp25", "tp26", "tp28", "tp29", "tp30", "tp31",
...
def update_overview_status():
    ...
    try:
        ...
        for machine in MACHINES:
            ...
            # Try update
            cursor.execute("""
                UPDATE field_overview_tp_status_l3 
            ...
            if cursor.rowcount == 0:
                cursor.execute("""
                    INSERT INTO field_overview_tp_status_l3 (machine_status)
                    VALUES (%s)
                """, (json.dumps(status_json),))

        conn.commit()
        print(f"Updated field_overview_tp_status_l3 at {time.strftime('%Y-%m-%d %H:%M:%S')}")
...
def main():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS field_overview_tp_status_l3 (
                machine_status JSONB
            );
        """)
        conn.commit()
        print("Ensured field_overview_tp_status_l3 table exists")
    except psycopg2.Error as e:
