#loop3 outfeed jamming deque countor 
...
#for outfeed jamming 
cld_history = defaultdict(deque)
BUCKET_WINDOW_OUTFEED = 25
...
# jamming outfeed counter logic 
def update_cld_history(machine_name, cld_count):
    now = datetime.now()
    history = cld_history[machine_name]

    # Add current cld_count
    history.append((now, cld_count))

    # Remove entries older than BUCKET_WINDOW seconds
    while history and (now - history[0][0]).total_seconds() > BUCKET_WINDOW_OUTFEED:
        history.popleft()

def get_mode_cld_count(machine_name):
    history = cld_history[machine_name]
    counts = [count for ts, count in history]
    if not counts:
        return 0  # default
    freq = Counter(counts)
    # Print the full frequency distribution for debugging
    #print(f"[{machine_name}] CLD Count Frequency (last {BUCKET_WINDOW_OUTFEED}s):")
   # for count_val, freq_val in freq.items():
   #     print(f"  Count: {count_val}, Frequency: {freq_val}")
    return freq.most_common(1)[0][0]  # returns cld_count with highest frequency
    ...
    while True:
        ...
        for machine_name in machine_names:
            if machine_name in zone_nodes:
                ...
                # ------------Machine Outfeed Counter logic -----------
                elif re.match(r"mc\d+_outfeed", machine_name):
                    update_cld_history(machine_name, cld_count)
                    mode_cld = get_mode_cld_count(machine_name)

                    if mode_cld > 1 and "bad" in orientation.lower():
                        new_status = "jamming & bad_orientation"
                    elif mode_cld > 1:
                        new_status = "jamming"
                        print(f"[{machine_name}] Mode CLD Count: {mode_cld}")
                    elif "bad" in orientation.lower():
                        new_status = "bad_orientation"
                    elif "good" in orientation.lower():
                        new_status = "Healthy"
