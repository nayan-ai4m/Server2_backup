...
# VALID_INFEED_MACHINES_RH = [f"mc{i}_infeed_rh" for i in range(17, 23)]
# VALID_INFEED_MACHINES_LH = [f"mc{i}_infeed_lh" for i in range(17, 23)]

VALID_INFEED_MACHINES_RH = [f"mc{i}_infeed_rh" for i in [17, 19, 21]]
VALID_INFEED_MACHINES_LH = [f"mc{i}_infeed_lh" for i in [18, 20, 22]]


VALID_OUTFEED_MACHINES = [f"mc{i}_outfeed" for i in range(17, 23)]
VALID_INFEED_MACHINES_JAMMING = [f"mc{i}_infeed_jamming" for i in range(17, 23)]
...
def format_camera_id(machine_name):
    ...
    if name == 'pre_taping_l3':
        return 'L3-Pre-TM'
    elif name == 'outfeed_L3C4_l3':
        return 'L3-Pre-TM'
    elif name == 'l3_outfeed':
        return 'L3-Pre-TM'
    elif name == 'l3_tapping_outfeed':
        return 'L3-TM'
    elif name == 'l3_tapping_postOutfeed':
        return 'L3-TM'
    elif name == 'check_weigher_rejection_l3':
        return 'L3_EOL'
    elif name == 'case_erector_conveyor_l3':
        return 'L3_EOL'
    elif name == 'highbay_l3':
        return 'L3_High-bay'
...
def format_event_type(status_or_orientation, machine_name):
    ...
    if status_or_orientation == 'jamming':
        ...
        return f"Bad orientation and Starvation detected in {mc_name} infeed"
    elif status_or_orientation == 'jamming & bad_orientation':
        return f"Bad orientation and Jamming detected in {mc_name} infeed"
    elif status_or_orientation == 'possible_starvation':
        return f"Possible starvation in {mc_name} infeed"
    elif status_or_orientation == 'possible_jamming':
        return f"Possible jamming in {mc_name} infeed"
...
def process_kafka_messages():
    ...
    for message in consumer:
        try:
            ...
            # Special Case: pre_taping_l3
            if machine_name == 'pre_taping_l3':
                if 'bad_orientation' in orientation:
                    event_type_text = 'Bad Mat Filling'
                    insert_event(machine_name, event_type_text, camera_id)
                continue

            # Special Case: outfeed_L3C4_l3
            #if machine_name == 'outfeed_L3C4_l3':
            #   if 'bad_orientation' in orientation:
            #        event_type_text = 'Bad Mat Filling'
            #        insert_event(machine_name, event_type_text, camera_id)
            #    continue

            # Special Case: l3_outfeed
            if machine_name == 'l3_outfeed':
            ...
            if machine_name == 'l3_tapping_outfeed':
                if 'bad_orientation' in orientation:
                    event_type_text = 'Bad taping detected'
                    insert_event(machine_name, event_type_text, camera_id)
                continue
            
            # Special Case: l3_tapping_postOutfeed
            # if machine_name == 'l3_tapping_postOutfeed':
                # if 'bad_orientation' in orientation:
                    # event_type_text = 'Bad taping detected'
                    # insert_event(machine_name, event_type_text, camera_id)
                # continue
            
            # Special Case: check_weigher_rejection_l3
            if machine_name == 'check_weigher_rejection_l3':
                if 'possible_jamming' in status:
                    event_type_text = 'Possible CLD jamming in Loop3 Check Weigher Rejection Zone'
                    insert_event(machine_name, event_type_text, camera_id)
                elif 'jamming' in status:
                    event_type_text = 'CLD Jamming in Loop3 Check Weigher Rejection Zone'
                    insert_event(machine_name, event_type_text, camera_id)
                continue
            
            # Special Case: case_erector_conveyor_l3
            if machine_name == 'case_erector_conveyor_l3':
                if 'possible_jamming' in status:
                    event_type_text = 'Possible CLD jamming in Case Erector Coveyor Loop 3'
                    insert_event(machine_name, event_type_text, camera_id)
                elif 'jamming' in status:
                    event_type_text = 'CLD Jamming in Case Erector Coveyor Loop 3'
                    insert_event(machine_name, event_type_text, camera_id)
                continue

            # Special Case: Loop 4 HighBay
            if machine_name == 'highbay_l3':
                if 'possible_jamming' in status:
                    event_type_text = 'Possible CLD jamming in Highbay Coveyor Loop 3'
                    insert_event(machine_name, event_type_text, camera_id)
                elif 'jamming' in status:
                    event_type_text = 'CLD Jamming in Highbay Coveyor Loop 3'
                    insert_event(machine_name, event_type_text, camera_id)
                continue

            # Special Case: Loop 4 HighBay
            if machine_name == 'cw_outfeed_l3':
                if 'possible_jamming' in status:
                    event_type_text = 'Possible CLD jamming in CW To highbay Coveyor Loop 3'
                    insert_event(machine_name, event_type_text, camera_id)
                elif 'jamming' in status:
                    event_type_text = 'CLD Jamming in CW To highbay Coveyor Loop 3'
                    insert_event(machine_name, event_type_text, camera_id)
                continue
            ...
            if machine_name_lower in VALID_INFEED_MACHINES_LH + VALID_INFEED_MACHINES_RH + VALID_OUTFEED_MACHINES:
                if status in ('jamming', 'starvation', 'possible_jamming', 'possible_starvation', 'bad_orientation',
                        'starvation & bad_orientation', 'jamming & bad_orientation', 'bad_orientation & starvation','bad_orientation & jamming'):
                    event_type_text = format_event_type(status, machine_name)
                    insert_event(machine_name, event_type_text, camera_id)
