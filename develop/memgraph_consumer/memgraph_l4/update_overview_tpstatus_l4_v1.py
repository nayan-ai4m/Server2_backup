...
def update_overview_status():
    ...
    try:
        ...
        for machine in MACHINES:
            ...
            cursor.execute("""
                UPDATE overview_tp_status_loop4 
            ...
            if cursor.rowcount == 0:
                cursor.execute("""
                    INSERT INTO overview_tp_status_loop4 (machine_status)
                    VALUES (%s)
                """, (json.dumps(status_json),))

        conn.commit()
        print(f"Updated overview_tp_status_loop4 at {time.strftime('%Y-%m-%d %H:%M:%S')}")
...
def main():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS overview_tp_status_loop4 (
                machine_status JSONB
            );
        """)
        conn.commit()
        print("Ensured overview_tp_status_loop4 table exists")
    except psycopg2.Error as e:
