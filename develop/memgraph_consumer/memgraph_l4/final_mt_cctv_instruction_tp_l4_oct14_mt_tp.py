...
def get_table(machine):
    ...
    # if machine == 'pre_taping_l4': return 'press_tp_status_loop4'
    if machine == 'outfeed_l4c4_l4': return 'press_tp_status_loop4'
    if machine == 'l4_outfeed': return 'press_tp_status_loop4'
    # if machine == 'l4_tapping_outfeed': return 'tpmc_tp_status_loop4'
    # if machine == 'l4_tapping_postoutfeed': return 'tpmc_tp_status_loop4'
    if machine == 'check_weigher_rejection_l4': return 'check_weigher_tp_status_loop4'
    if machine == 'highbay_l4': return 'highbay_tp_status_loop4'
    if machine == 'cw_outfeed_l4': return 'highbay_tp_status_loop4'
    if machine == 'case_erector_conveyor_l4': return 'case_erector_tp_status_loop4'
    if machine.startswith('mc'): return f"mc{''.join(filter(str.isdigit, machine.split('_')[0]))}_tp_status"

def get_tp_columns(part):
    part = part.lower()
    return {
        'infeed': ['tp08', 'tp13', 'tp42', 'tp43', 'tp48', 'tp49'],  #tp15
        'preinfeed': ['tp09', 'tp14'],
        'postoutfeed': ['tp10'],
        'outfeed': ['tp11'],
        'dropping_zone': ['tp12'],
...
def get_tp_column(status, orient, part):
    ...
    # if part == 'l4_tapping_outfeed' and ('bad_orientation' in [status, orient]): return 'tp24'
    # if part == 'l4_tapping_postoutfeed' and ('bad_orientation' in [status, orient]): return 'tp24'
    #if part == 'pre_taping_l4' and ('bad_orientation' in [status, orient]): return 'tp31'
    if part == 'outfeed_l4c4_l4' and ('bad_orientation' in [status, orient]): return 'tp31'
    if part == 'l4_outfeed' and ('jamming' in status) and ('possible_jamming' not in status): return 'tp32'
    if part == 'check_weigher_rejection_l4' and ('jamming' in status) and ('possible_jamming' not in status): return 'tp28'
    if part == 'case_erector_conveyor_l4' and ('jamming' in status) and ('possible_jamming' not in status): return 'tp40'
    if part == 'highbay_l4' and ('jamming' in status) and ('possible_jamming' not in status): return 'tp30' 
    if part == 'cw_outfeed_l4' and ('jamming' in status) and ('possible_jamming' not in status): return 'tp30'
    ...
    if 'bad_orientation' in status and part == 'infeed': return 'tp42' if 'starvation' in status else 'tp43' if 'jamming' in status else None #'tp15'
    if 'bad_orientation' in status and part == 'outfeed': return 'tp43' if 'jamming' in status else None #'tp15'
...
def update_status(machine, status, orient):
    try:
        part = machine.lower()
        if part not in [ "l4_outfeed", "check_weigher_rejection_l4", "case_erector_conveyor_l4","highbay_l4","cw_outfeed_l4"]:   #'l4_tapping_outfeed', 'pre_taping_l4','l4_tapping_postoutfeed'
        ...
        if status and status.lower() == 'healthy':
            with get_db_conn() as conn, conn.cursor() as cur:
                ...
                for col in get_tp_columns(part):
                    # 🛑 Skip clearing tp24 for l4_tapping_outfeed
                    # if machine.lower() == 'l4_tapping_outfeed' and col == 'tp24':
                    ...
                    if machine.lower() == 'pre_taping_l4' and col == 'tp31':
                        cur.execute(f"SELECT {col} FROM {table} WHERE {col}->>'machine_part' = %s LIMIT 1", (part,))
                        res = cur.fetchone()
                        if res and res[0]:
                            data = json.loads(res[0]) if isinstance(res[0], str) else res[0]
        ...
        with get_db_conn() as conn, conn.cursor() as cur:
            ...
            if existing and existing[0]:
                ...
                if machine.lower() == 'l4_tapping_outfeed' and col == 'tp24' and existing_data.get("active") == 1:
                ...
                elif machine.lower() == 'pre_taping_l4' and col == 'tp31' and existing_data.get("active") == 1:
                    # Just refresh timestamp and update
                    existing_data["timestamp"] = datetime.now().isoformat()
                    cur.execute(f"UPDATE {table} SET {col} = %s WHERE {col}->>'machine_part' = %s", (json.dumps(existing_data), part))
                    print(f"Refreshed timestamp for {col} of {machine}")
