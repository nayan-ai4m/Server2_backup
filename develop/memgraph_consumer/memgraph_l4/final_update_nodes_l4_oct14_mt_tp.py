...
KAFKA_TOPIC_1 = "cctv_l4_1"  
KAFKA_TOPIC_2 = "cctv_l4_2"
KAFKA_TOPIC_3 = "cctv_l4_3"
...
def fetch_latest_kafka_data(db, machine_name):
    result = list(db.execute_and_fetch(
        """
        MATCH (m:kafkaData_l4 {name: $machine_name})
...
def update_machine_status(db, machine_name, new_status, orientation, cld_status, timestamp, source_id, cld_count):
    db.execute("""
        MATCH (m:zone_l4 {name: $machine_name})  
...
def check_jamming_starvation(status, current_timestamp):
    ...
    if "temp" in status:
        if elapsed >= timedelta(seconds=40):
...
BAD_ORIENTATION_DURATION = 3  # seconds
    ...
    while True:
        ...
        for machine_name in machine_names:
            if machine_name in zone_nodes:
                ...
                # ------------Machine Infeed-----------
                # if machine_name.startswith("mc") and machine_name.endswith("_infeed_lh"):
                ...
                # ------------Machine Full Infeed  zone -----------
                if machine_name.startswith("mc") and machine_name.endswith("_infeed_jamming"):
                    if cld_count == 0:
                        new_status = "no_cld"
                ...
                # ------------Taping Machine-----------
                # elif machine_name=="l4_tapping_outfeed":
                #     if "bad" in orientation.lower():
                #         new_status = "bad_orientation"
                #     elif "good" in orientation.lower():
                #         new_status = "tttt"
                ...
                # elif machine_name=="l4_tapping_postOutfeed":
                    # if "bad" in orientation.lower():
                        # new_status = "bad_orientation"
                    # elif "good" in orientation.lower():
                        # new_status = "Healthy"

                elif machine_name=="l4_outfeed":
                    if cld_count > 5:
                        new_status = "temp"
                    elif "good" in orientation.lower():
                        new_status = "Healthy"
                ...
                #elif machine_name=="outfeed_L4C4_l4":
                #    if "bad" in orientation.lower():
                #        new_status = "bad_orientation"
                #    elif "good" in orientation.lower():
                #        new_status = "Healthy"
                ...
                # ------------EOL-----------
                elif machine_name== "case_erector_conveyor_l4":
                    if cld_count > 5:
                        new_status = "temp"
                    else:
                        new_status = "Healthy"

                elif machine_name== "check_weigher_rejection_l4":
                    if cld_count > 4:
                        new_status = "temp"
                    else:
                        new_status = "Healthy"
                ...
                # elif machine_name== "cw_outfeed_l4":
                #     if cld_count >= 2:
                #         new_status = "temp"
                #     else:
                #         new_status = "Healthy"
               ...
               # Getting the Current Status of the Node/Zone from Memgraph to check with the New Status
                ...
                # Prevent downgrading from jamming/sravation to possible/temp/np_cld/infeedj
