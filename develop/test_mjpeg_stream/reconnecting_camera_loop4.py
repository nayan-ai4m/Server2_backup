...
log_file = os.path.join(log_dir, f"camera_stream_{datetime.datetime.now().strftime('%Y%m%d')}.log")
...
def main():
    ...
    try:
        ...
        cam_stream = Reconnecting_CamGear(
            cam_address="rtsp://admin:unilever2024@192.168.1.28:554/Streaming/Channels/102",
            reset_attempts=2000,  
            reset_delay=5,
        )
        
        try:
            stream = Stream("loop4", size=(1280, 720), quality=50, fps=20)
            server = MjpegServer("0.0.0.0", 3007)
