...
from database_test import get_db, close_db, get_server2_db, close_server2_db
from database_test import get_device_db, close_db
...
leakage_key = {
    "suggestion_lekage_key": ["HMI_Hor_Seal_Front_27","HMI_Hor_Seal_Rear_28","HMI_Hor_Sealer_Strk_1","HMI_Hor_Sealer_Strk_2"]
}
...
async def get_prediction_table_data():
    try:
        ...
        response = {
            "prediction_overview": prediction_data
        }
        return response
    except Exception as e:
        print(f"Error in /predictiontabledata endpoint: {e}")
        print(traceback.format_exc())
        raise HTTPException(status_code=500, detail="Error fetching prediction table data")
...
async def save_machineDegreeData(data: MachineDegreeData):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error saving machine degree: {str(e)}")
 ## Laminate settings 
...
async def save_leakage_suggestion(data: SuggestionData):
    try:
        ...
        print("Error:", e)
        print(traceback.format_exc())
        raise HTTPException(status_code=500, detail=f"Error processing leakage suggestion: {str(e)}")   
...
async def save_ignored_suggestion(request: Request):
    try:
        ...
        print("Error:", e)
        print(traceback.format_exc())
        raise HTTPException(status_code=500, detail=f"Error processing ignored suggestion: {str(e)}")
...
async def get_cockpit_suggetions(
    machine_number: str = Query(..., pattern="^mc(17|18|19|20|21|22|25|26|27|28|29|30)$"),
    role: Optional[str] = Query(default=None)
):
    try:
...
@app.get("/latest-gsm")
async def get_latest_gsm_endpoint(machine_number: str = Query(..., pattern="^mc(17|18|19|20|21|22|25|26|27|28|29|30)$")):
    try:
        ...
        print(f"Error: {e}")
        print(traceback.format_exc())
        raise HTTPException(status_code=500, detail=f"Error fetching latest GSM data: {str(e)}")
...
async def get_historical_work_instructions(
    ...
    machine_number: str = Query(..., pattern="^(mc(17|18|19|20|21|22|25|26|27|28|29|30)|all|mcckwr3|ckwr3|ckwr4|mcckwr4|mctp3|mctp4|tpmc3|tpmc4|mcce3|ce3|mcce4|ce4|mchighbay3|mchighbay4|hig    hbay3|highbay4|mcpress3|press3|mcpress4|press4)$")):  
