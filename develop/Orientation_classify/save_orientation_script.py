...
class SimpleImageClassifier:
    def __init__(self, input_folder: str, triton_url: str = "http://100.103.195.124:8007"):
        ...
        # Create cropping images output directory
        self.cropping_output_dir = "/home/ai4m/develop/Orientation_classify/crooping_images"
        os.makedirs(self.cropping_output_dir, exist_ok=True)
        print(f"📁 Cropped images will be saved to: {self.cropping_output_dir}")
    ...
    def parse_image_filename(self, filename: str) -> Optional[ImageData]:
        ...
        # Pattern for: mc{prefix}raw_{class_id}_{x}_{y}_{x+w}_{y+h}_{timestamp}.png
        pattern = r'mc(\d+)_raw_(\d+)_(\d+\.?\d*)_(\d+\.?\d*)_(\d+\.?\d*)_(\d+\.?\d*)_(\d{4}_\d{2}_\d{2}T\d{2}_\d{2}_\d{2})(?:_(\d+))?\.png'
    ...
    def save_cropped_image_for_bad_orientation(self, image_path: str, image_data: ImageData) -> str:
        """Save the cropped image for bad orientation detection"""
        try:
            # Load the full image
            ...
            cropped_filename = f"mc{image_data.mc_id}_bad_cropped_{timestamp_str}_{image_data.class_id}.png"
            ...
            print(f"  💾 Saved bad orientation cropped image: {cropped_filename}")
            return cropped_path
            
        except Exception as e:
            print(f"  ⚠️ Error saving cropped image for bad orientation: {e}")
            return ""
    ...
    def save_bad_orientation_image(self, image_path: str, image_data: ImageData, output_dir: str = "/home/ai4m/develop/data/baumer/infeed_orientation/") -> str:
        ...
        timestamp_str = image_data.timestamp.strftime('%Y%m%d_%H%M%S')
        output_filename = f"mc{image_data.mc_id}_bad_orientation_{timestamp_str}_{image_data.class_id}.png"
    ...
    async def process_image(self, image_data: ImageData) -> dict:
        ...
        try:
            ...
            if classification in ["bad_sleeping", "bad_standing"]:
                ...
                # *** SAVE IMAGES ONLY FOR BAD ORIENTATION ***
                try:
                    # Save image with bounding box
                    saved_image_path = self.save_bad_orientation_image(image_path, image_data)
                    # Save cropped image 
                    cropped_image_path = self.save_cropped_image_for_bad_orientation(image_path, image_data)
                except Exception as e:
                    print(f"  ⚠️ Failed to save bad orientation images: {e}")
                    saved_image_path = ""
                    cropped_image_path = ""
            ...
            result = {
                ...
                'cropped_image_path': cropped_image_path,  # Path to saved cropped image
...
if __name__ == "__main__":
    ...
    print("💾 Cropped images folder: /home/ai4m/develop/Orientation_classify/crooping_images")
