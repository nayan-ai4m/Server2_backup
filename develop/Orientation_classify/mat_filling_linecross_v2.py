...
class MatClassifier:
    ...
    def get_model_name(self, loop_id: str) -> str:
        ...
        if loop_id == "pre_taping_l3":
            return "l3_mat_classification_model"
        elif loop_id == "pre_taping_l4":
            return "l4_mat_classification_model"
        else:
    ...
    async def check_triton_status(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "l3_mat_classification_model" not in model_names:
                    print("❌ ERROR: l3_mat_classification_model not found!")
                if "l4_mat_classification_model" not in model_names:
                    print("❌ ERROR: l4_mat_classification_model not found!")
    ...
    async def infer_with_triton(self, image_data: np.ndarray, model_name: str) -> tuple[str, float]:
        ...
        try:
            ...
            # ----------- LOGITS PROCESSING (UPDATED) -----------
            logits = result["outputs"][0]["data"]
            logits = np.array(logits, dtype=np.float32)

            print("Raw logits:", logits)

            # Case 1: TWO LOGITS (bad, good)
            if len(logits) == 2:
            ...
            # Case 2: SINGLE LOGIT (binary)
            elif len(logits) == 1:
                prob = float(1 / (1 + np.exp(-logits[0])))

                classification = "good" if prob >= 0.5 else "bad"
                confidence = prob if classification == "good" else (1.0 - prob)

            else:
                print(f"  ⚠️ Unexpected output size: {len(logits)}")
                return "unknown", 0.0

            confidence = float(confidence)
            print(f"Predicted: {classification}, Confidence: {confidence:.4f}")
