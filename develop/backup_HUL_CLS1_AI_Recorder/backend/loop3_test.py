...
class Loop3Monitor:
    ...
    def get_machine_data(self, mc, json_key, start_time, end_time):
        ...
        ###
            ...
            buffer_time = current_time - timedelta(minutes=3)
            print("Buffer Time : ",buffer_time)
        
            """
            self.db.cursor.execute(f"SELECT timestamp, status FROM mc{mc} ORDER BY timestamp DESC LIMIT 1")
            row = self.db.cursor.fetchone()
            if row:
                db_timestamp, status = row
                db_timestamp = db_timestamp.replace(tzinfo=None)  

                if db_timestamp >= buffer_time:
                    # Status has been updated within the last minute
                    if status == 1:
                        status_message = "RUNNING"
                    else:
                        status_message = "STOPPED"
                else:
                    # No status change in the last minute, mark as offline
                    status_message = "OFFLINE"
            else:
                status_message = "OFFLINE"  # No data, consider it offline
        ...
        """)
            ...
            #print("Machine Row:",machine_row)
            stoppage_time = self.calculate_current_shift_duration(mc)

            if machine_row:
                db_timestamp = machine_row[2]
                # machine_timestamp = db_timestamp.replace(tzinfo=None)
                if isinstance(machine_row[2], float):
                # If machine_row[2] is in string format, convert it to datetime
                machine_timestamp = datetime.strptime(machine_row[2], '%Y-%m-%d %H:%M:%S')  # Use the correct format here
                else:
                    # If it's already a datetime object, you can directly use it
                    machine_timestamp = machine_row[2]
                    print("Machine Timestamp:",machine_timestamp, "machine row values:",machine_row[1],type(machine_row[1])) 
                if machine_timestamp >= buffer_time:

                    print("Machine Row: ",machine_row[1])
                    if machine_row[1] == 1 :
                        status_message = "Running"
                        print("Inside If",status_message)
                    else :
                        print("Inside else machien stopped")
                        status_message = "Stopped"
                #elif machine_timestamp < buffer_time:
                #    status_message = "STOPPED"
                #    print("Inside ElseIf",status_message)
                else:
                    print("inside main else offline message")
                    status_message = "Offline"
                    
                hopper_level_primary = round(machine_row[4], 2) if machine_row[4] is not None else 78.43
                #print("Hopper level primary:",hopper_level_primary)
                hopper_level_secondary = round(machine_row[5], 2) if machine_row[5] is not None else 30.4 if mc == 18 else 33.5
                #print("Hopper Level Secondary:",hopper_level_secondary)
    ...
    def update_loop3_overview(self):
        ...
        try:
           ...
           # print("Macine data:",machines_data)
