...
@try_export
def export_onnx_end2end(model, im, file, simplify, topk_all, iou_thres, conf_thres, device, labels, fixed_input=False, prefix=colorstr('ONNX END2END:')):
    # YOLO ONNX export
    check_requirements('onnx')
    import onnx
    LOGGER.info(f'\n{prefix} starting export with onnx {onnx.__version__}...')
    ...
    # Fixed input size: only batch is dynamic; Dynamic input: batch, height, width are dynamic
    if fixed_input:
...
def run(
        ...
        fixed_input=False,  # ONNX END2END: fixed input size
):
    t = time.time()
    ...
    if onnx_end2end:
        if isinstance(model, DetectionModel):
            labels = model.names
            f[2], _ = export_onnx_end2end(model, im, file, simplify, topk_all, iou_thres, conf_thres, device, len(labels), fixed_input)
        else:
...
def parse_opt():
    ...
    parser.add_argument('--fixed-input', action='store_true', help='ONNX END2END: fixed input size (only batch dynamic)')
    parser.add_argument(
        '--include',
        nargs='+',
        default=['torchscript'],
