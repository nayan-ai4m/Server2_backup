...
extern "C" void analytics_custom_parse_nvdsanalytics_meta_data(
  ...
  std::string save_path = "/home/ai4m/develop/data/cam/";  // Default path
  
  if (meta->objLCCurrCnt["mc25_cld"]) {
    mc_prefix = "mc25_";
    zone_name = "mc25_infeed_jamming";
  } else if (meta->objLCCurrCnt["mc26_cld"]) {
    mc_prefix = "mc26_";
    zone_name = "mc26_infeed_jamming";
  } else if (meta->objLCCurrCnt["mc27_cld"]) {
    mc_prefix = "mc27_";
    zone_name = "mc27_infeed_jamming";
  } else if (meta->objLCCurrCnt["mc28_cld"]) {
    mc_prefix = "mc28_";
    zone_name = "mc28_infeed_jamming";
  } else if (meta->objLCCurrCnt["mc29_cld"]) {
    mc_prefix = "mc29_";
    zone_name = "mc29_infeed_jamming";
  } else if (meta->objLCCurrCnt["mc30_cld"]) {
    mc_prefix = "mc30_";
    zone_name = "mc30_infeed_jamming";
  } else if (meta->objLCCurrCnt["l4_tapping"]) {
    mc_prefix = "l4_taping_";
    zone_name = "l4_tapping_outfeed";
    save_path = "/home/ai4m/develop/data/taping_cam/";
  }
	  ...
	  for (l = frame_meta->obj_meta_list; l != NULL; l = l->next) {
			...
			for (NvDsMetaList *l_user_meta = obj_meta->obj_user_meta_list; l_user_meta != NULL;
        		...
        		if (line_crossed && object_in_zone) {
                    ...
                    snprintf(userData.fileNameImg,FILE_NAME_SIZE,"%s%sraw_%d_%f_%f_%f_%f_%s.png",save_path.c_str(),mc_prefix.c_str(),class_id,x,y,x+w,y+h,time_stamp.str().c_str());
