	  ...
	  for (l = frame_meta->obj_meta_list; l != NULL; l = l->next) {
			...
			for (NvDsMetaList *l_user_meta = obj_meta->obj_user_meta_list; l_user_meta != NULL;
        		...
        		if (line_crossed && object_in_zone) {
                    ...
                    snprintf(userData.fileNameImg,FILE_NAME_SIZE,"/home/ai4m/develop/data/cam/%sraw_%d_%f_%f_%f_%f_%s.png",mc_prefix.c_str(),class_id,x,y,x+w,y+h,time_stamp.str().c_str());
