...
generate_dsmeta_message_minimal(void* privData, void* frameMeta)
{
    ...
    */
    ...
    JsonNode* rootNode;
    JsonObject* jobject;
    JsonObject* linecrossingdata = json_object_new ();
    JsonObject* roidata = json_object_new ();
    JsonArray* jArray;
    stringstream ss;
    gchar* message = NULL;
    ...
    for (NvDsObjectMetaList* obj_l = frame_meta->obj_meta_list; obj_l; obj_l = obj_l->next) {
        ...
        ss.str("");
        ss.clear();
	if (frame_meta->filename != NULL )
	{
        ss << obj_meta->object_id << "|" <<obj_meta->confidence << "|" << left << "|" << top << "|" << left + width << "|" << top + height << "|" << obj_meta->obj_label<< "|" <<frame_meta->filename<<"|";
	}
	else{
		ss << obj_meta->object_id << "|" <<obj_meta->confidence << "|" << left << "|" << top << "|" << left + width << "|" << top + height << "|" << obj_meta->obj_label<< "|" <<"none"<<"|";
	}
	for (NvDsMetaList *l_user_meta = obj_meta->obj_user_meta_list; l_user_meta != NULL;
		...
		if(user_meta->base_meta.meta_type == NVDS_USER_OBJ_META_NVDSANALYTICS){
			...
			if (user_meta_data->roiStatus.size() > 0){
				for (std::string label : user_meta_data->roiStatus) {
        				ss << label << "|";
    				}
			}
			if (user_meta_data->lcStatus.size() > 0){
                                for (std::string label : user_meta_data->lcStatus) {
                                        ss << label << "|";
                                }
                        }
    ...
    json_object_set_string_member(jobject, "@timestamp", ts);
     ...
     //json_object_set_string_member(jobject, "frameMeta", frame_meta->filename);
    ...
    for (NvDsMetaList * l_user = frame_meta->frame_user_meta_list;
            ...
            for (std::pair<std::string, uint32_t> status : meta->objLCCurrCnt){
                json_object_set_int_member(linecrossingdata,status.first.c_str(),status.second);
            }	
            for (std::pair<std::string, uint32_t> status :meta->objInROIcnt ){
                json_object_set_int_member(roidata,status.first.c_str(),status.second);
            }

        }
    json_object_set_object_member (jobject, "linecross",linecrossingdata);
    json_object_set_object_member (jobject, "roi",roidata);
    ...
    json_node_free(rootNode);
    //json_object_unref(linecrossingdata);
    //json_object_unref(roidata);
